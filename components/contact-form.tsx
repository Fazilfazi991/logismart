...
export function ContactForm() {
  ...
  return <form noValidate onSubmit={submit} className="grid gap-5 rounded-[20px] border border-border bg-white p-6 shadow-[var(--shadow-card)] md:p-8">
    <div className="grid gap-5 md:grid-cols-2"><Field label="Full Name" required /><Field label="Company Name" required /><Field label="Email Address" type="email" required /><Field label="Phone Number" type="tel" required /><label className="grid gap-2 text-sm font-bold text-primary md:col-span-2"><span>Service Required</span><select className="field" required defaultValue=""><option value="" disabled>Select a service</option>{serviceOptions.map((service) => <option key={service}>{service}</option>)}</select></label></div>
